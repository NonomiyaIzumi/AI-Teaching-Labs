...
**Bản TensorFlow/Keras:** `keras-tensorflow/02_Debug-gradient-Gradient-Checking/02_Kiem_tra_gradient_Keras.ipynb` — bài học độc lập về autodiff (`tf.GradientTape`) và gradient checking. Cũng có bản gộp cả 5 chủ đề Module 1 thành 1 file: `keras-tensorflow/Pima_Diabetes_Nen_Tang_Deep_Learning_Keras.ipynb`. Xem `keras-tensorflow/README.md` để biết cách cài đặt/chạy local.

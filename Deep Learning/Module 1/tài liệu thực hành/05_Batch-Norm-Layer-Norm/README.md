...
**Bản TensorFlow/Keras (minh hoạ dùng framework thật):** `keras-tensorflow/05_Batch-Norm-Layer-Norm/05_Batch_Norm_Layer_Norm_Keras.ipynb` — thay `batchnorm_forward/backward` và `layernorm_forward/backward` tự viết bằng lớp có sẵn `layers.BatchNormalization()`/`layers.LayerNormalization()`; xem `keras-tensorflow/README.md` để biết cách cài đặt/chạy local.

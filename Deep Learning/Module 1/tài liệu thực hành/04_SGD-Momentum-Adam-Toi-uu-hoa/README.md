...
**Bản TensorFlow/Keras (minh hoạ dùng framework thật):** `keras-tensorflow/04_SGD-Momentum-Adam-Toi-uu-hoa/04_Toi_uu_hoa_Keras.ipynb` — thay `update_parameters_with_gd/momentum/adam` và `random_mini_batches` tự viết bằng optimizer object có sẵn (`keras.optimizers.SGD`/`Adam`) và `model.fit(..., batch_size=64)`; xem `keras-tensorflow/README.md` để biết cách cài đặt/chạy local.

...
**Bản TensorFlow/Keras (minh hoạ dùng framework thật):** `keras-tensorflow/01_Kien-truc-DNN-va-Backpropagation/01_Xay_dung_mo_hinh_DNN_Keras.ipynb` — cùng bài toán và bộ dữ liệu, nhưng dùng `tf.keras.Sequential`/`model.fit()` thay vì tự cài forward/backward bằng NumPy; xem `keras-tensorflow/README.md` để biết cách cài đặt/chạy local (`uv sync` trong `keras-tensorflow/`).

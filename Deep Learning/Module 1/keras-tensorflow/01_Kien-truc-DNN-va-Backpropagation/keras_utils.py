...
Module 1, de sinh vien de doi chieu; compile_and_train()/evaluate_classification() la
phan "hau ky" (compile, fit, danh gia) khong doi giua cac bai, nen dat san o day thay vi
bat sinh vien lam lai moi bai.
"""
...
def load_dataset(csv_path="../../data/pima-indians-diabetes.csv", test_size=0.2, seed=1):
    ...
    Luu y: day la quy uoc (n_x, m) cua NumPy tu dau, KHAC voi quy uoc sample-first (m, n_x)
    ma Keras mong doi - can .T truoc khi dua vao model.fit()/model.predict() (xem compile_and_train).
    """
...
def compile_and_train(model, X_train, Y_train, optimizer, epochs=100, batch_size=None, verbose=0):
    ...
    batch_size=None nghia la full-batch gradient descent (giong cac bai 01/03 NumPy);
    dat batch_size cu the (vd 64) de Keras tu chia mini-batch (giong random_mini_batches bai 04).
    """
...
def evaluate_classification(model, X, y, title):
    ...
    Ban tuong duong (dung Keras thay vi forward_propagation() tu viet tay) cua evaluate_classification
    o cac bai NumPy from-scratch.
    """
